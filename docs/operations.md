...
### Recognition comparison MVP

The hosted speech image bundles multilingual Whisper **small** and Silero VAD v6.2.0 (both checksum verified). The Whisper service must use `/opt/minisago-models/ggml-small.bin`, `--vad-model /opt/minisago-models/ggml-silero.bin`, and a 2 GB memory limit. Remove `--no-timestamps` to retain segment timing. Requests enable speech detection by default and request verbose diagnostics. Decoder temperature fallback is disabled for reproducible comparisons; temperature remains adjustable per profile.

In browser mode, **Compare recordings without a Codex reply** is on by default. Record and send a clip to save it and automatically run both editable profiles. Adjust language, beam size, temperature, vocabulary hints, or VAD settings and select **Run both profiles** to replay the original audio. Turn this checkbox off to use the conversation path; that recording is still saved for later replay. Recording is manually started/submitted; Silero detects and filters speech on the server, rather than automatically ending microphone capture.

Recordings, expected transcripts, profile settings, and results are stored in `/app/state/voice-tests` (`MINISAGO_VOICE_TEST_DIR` overrides this). The existing state volume preserves them across deploys and logouts. All holders of the dedicated dashboard token share this test library. Up to 100 recordings of 30 seconds each and 20 runs per recording are retained; delete recordings explicitly from the dashboard. Clear events clears only transient traces. Comparison jobs run sequentially, at most one batch at a time; interrupted jobs can be rerun after a restart. Comparisons use Whisper only, without invoking Codex or Discord playback.

On the current two-thread ARM service, a synthetic short speech sample took approximately 35–44 seconds per small-model profile, including conversion, VAD, decoding, and language diagnostics. Recognition defaults to a 120-second deadline. These are diagnostic benchmark results, not a realtime latency claim.
